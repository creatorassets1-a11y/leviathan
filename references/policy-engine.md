...
The policy engine converts prose requirements into state, checks, evidence, and release gates.
It is designed to work even when the host has no dedicated Leviathan runtime.
...
`high` blocks security-sensitive release by default and requires explicit risk acceptance
for non-security categories.
`medium` is tracked and must have an owner and remediation plan.
`low` is documented debt unless the project has stricter policy.

Never combine security, accessibility, and dependency findings into one arbitrary count.
Each category has its own release rule.
