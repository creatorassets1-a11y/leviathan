...
4. Treat `.leviathan/state.json`, `.leviathan/evidence/ledger.json`, and `.leviathan/skills.lock.json` as gate/provenance records when they exist. Never mark a gate passed without executed evidence.
...
9. Prefer Leviathan-native policy and official technology-maker skills over generic third-party guidance. Popularity is not a trust signal.
10. Preserve `PRD.md`, `DESIGN.md`, `RUNBOOK.md`, `SECURITY.md`, `DECISIONS.md`, and evidence artifacts for production builds.
11. Keep canonical policy in `LEVIATHAN.md`; do not fork vendor-specific versions of it.
...
14. Before declaring done, run the project's relevant tests and `node tools/leviathan-check.mjs` when the checker is available. Report exact failures and limitations.
